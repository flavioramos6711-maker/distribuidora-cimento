...
// Componente interativo com carrinho, avaliações, galeria, etc.
// Recebe dados do servidor via props (hydratação limpa)
// =============================================================================
...
import { ProductStructuredData } from "@/components/store/seo-structured-data"
...
export function ProductPageClient({ product: initialProduct }: { product: Product }) {
  ...
  const { data: product } = useSWR(`product-${slug}`, async () => {
    const { data } = await supabase
      ...
      .single()
    return data
  }, { fallbackData: initialProduct, revalidateOnMount: false, revalidateOnFocus: false })
  ...
  const ratingCounts = [5, 4, 3, 2, 1].map((star) => ({
    star,
    count: reviews && reviews.filter((r: Review) => r.rating === star).length || 0,
  }))
  ...
  return (
    <>
      ...
      <ProductStructuredData
        ...
        rating={reviews && reviews.length > 0 ? Number(avgRating) : undefined}
        reviewCount={reviews?.length}
        url={`${SITE.siteUrl}/produto/${product.slug}`}
      />

      <div className="mx-auto max-w-7xl px-4 py-6 sm:py-10 pb-28 md:pb-10">
        {/* Breadcrumb */}
        <nav
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="lg:col-span-5 space-y-7">
            <div>
              ...
              {/* Preço — visível no HTML inicial para Google Ads */}
            ...
            <div className="space-y-5">
              ...
              {/* Botões de ação — WhatsApp é o CTA primário */}
              <div className="flex flex-col gap-3">
                <a
                  ...
                  className={cn(
                    ...
                    "bg-[#25D366] text-lg font-bold text-white",
                    "shadow-[0_16px_36px_-10px_rgba(37,211,102,0.7)]",
                    "transition-all hover:bg-[#1EBE5B] hover:scale-[1.02] active:scale-95",
                    "btn-whatsapp-track"
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    "border-2 border-[#002D5B]/25 bg-white text-base font-bold text-[#002D5B]",
                    "transition-all hover:border-[#002D5B]/60 hover:bg-slate-50 active:scale-95",
      ...
      {/* ── Sticky Bottom Bar Mobile — WhatsApp direto ── */}
      <div className="fixed bottom-0 inset-x-0 z-[90] border-t border-slate-200 bg-white/95 backdrop-blur-md shadow-[0_-8px_30px_rgba(0,0,0,0.08)] md:hidden">
        <div className="flex items-center gap-3 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))]">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-lg font-black text-[#002D5B] leading-tight">
              R$ {Number(product.price).toFixed(2).replace(".", ",")}
              <span className="ml-1 text-xs font-bold text-slate-400">/{product.unit}</span>
            </p>
          </div>
          <a
            href={waLink(`Olá! Quero cotar ${product.name} (${qty} x R$ ${Number(product.price).toFixed(2).replace(".", ",")}) — vi no site e preciso de entrega. `)}
            target="_blank"
            rel="noopener noreferrer"
            data-track="btn_whatsapp"
            data-button-name="btn_whatsapp"
            data-source="product_page"
            onClick={() => trackWhatsAppClick("product_page", `/produto/${product.slug}`)}
            className="btn-whatsapp-track inline-flex h-[52px] shrink-0 items-center justify-center gap-2 rounded-2xl bg-[#25D366] px-5 text-sm font-bold text-white shadow-lg shadow-green-500/30 transition-all active:scale-95"
          >
