  ...
  return (
    <section className="relative overflow-hidden bg-white">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-0 lg:divide-x divide-white/10">
            {pillars.map((p) => (
              <div key={p.label} className="flex flex-col items-center lg:items-start gap-1 px-0 lg:px-8 text-center lg:text-left group">
                <p className="text-3xl sm:text-4xl font-black text-white tracking-tight leading-none" style={{ color: p.accent === "#F47920" ? "#F47920" : "white" }}>
                  {p.stat}
                </p>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">{p.statLabel}</p>
                <p className="text-xs text-white/30 font-medium mt-1 hidden sm:block">{p.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative py-20 sm:py-28">
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-primary/5 rounded-full blur-[80px] -ml-32 -mb-32 opacity-50" />
          ...
          <div className="flex flex-col lg:flex-row gap-16 xl:gap-24 items-start">
            ...
            <div className="flex-1 space-y-8 max-w-xl">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Nossa Identidade</span>
                </div>
                <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-[#00213F] tracking-tight leading-[1.05]">
                  Compromisso com a{" "}
                  <span className="text-primary italic">sua obra.</span>
                </h2>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-2">
                <Link
                  href="/produtos"
                  className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#00213F] px-7 text-sm font-black text-white hover:bg-primary transition-colors"
                >
                  Ver catálogo <ArrowRight className="h-4 w-4" />
                </Link>
                <Link
                  href="/sobre"
                  className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 px-7 text-sm font-black text-slate-600 hover:border-primary hover:text-primary transition-colors"
                >
                  Sobre nós
                </Link>
              </div>
            ...
            <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-5">
              {pillars.map((p) => (
                <div
                  key={p.label}
                  className="group relative bg-white p-7 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-primary/20 hover:-translate-y-1 transition-all duration-300 overflow-hidden"
                >
                  <div
                  ...
                  <div className="space-y-2">
                    <h3 className="text-base font-black text-[#00213F] tracking-tight">{p.label}</h3>
